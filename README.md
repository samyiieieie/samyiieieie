<div align="center">

# ˚ʚ hi, i'm sam ɞ˚

*CS student · 2nd year · always on matcha*

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=15&pause=1000&color=FFB6C1&center=true&vCenter=true&width=435&lines=building+things+that+matter+%F0%9F%92%AB;still+figuring+it+out+but+that%27s+ok)](https://git.io/typing-svg)

</div>

---

### ✦ about me

```python
sam = {
  "studying"  : ["Computer Science", "a little bit of everything"],
  "building"  : ["E-Locate – Iloilo food trail PWA", "Panikasog – disaster prep app"],
  "interests" : ["civic tech", "UI/UX", "algorithms (send help)"],
  "currently" : "surviving finals + pitching to anyone who'll listen",
}
```
---

### ✦ projects

🆘 **Panikasog**
> disaster preparedness mobile app built for Panay Island, with GPS check-in + gamification
---

### ✦ what i'm working on

🍜 **E-Locate / Iloilo Culinary Discovery Platform**
> a gamified food trail app for tourists exploring Iloilo's food scene

🧾 **Beacon**
> a Flutter-based POS and inventory management system with customizable role-based access control (RBAC) for flexible business operations

---

### ✦ stack & tools

![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat-square&logo=dart&logoColor=white)
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat-square&logo=flutter&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visual-studio-code&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)

---

### ✦ github stats

<div align="center">

![Sam's GitHub stats](https://github-readme-stats-eight-iota-16.vercel.app/api?username=samyiieieie&show_icons=true&theme=rose_pine&hide_border=true&bg_color=00000000)

![GitHub Streak](https://streak-stats.demolab.com?user=samyiieieie&theme=rose-pine&hide_border=true&background=00000000)

</div>

---

### ✦ a little more

- 💻 building and experimenting with software projects
- ☕ running on ideas, deadlines, and caffeine
- 🎯 interested in user-centered design and product thinking

---

<div align="center">

*thanks for stopping by ˘ᵕ˘*

</div>
